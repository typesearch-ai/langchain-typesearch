...
def _no_key_from_the_environment(monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest) -> None:
    # Las pruebas no leen la clave ni la URL de quien las corre, salvo las de integración en vivo
    # (TYPESEARCH_LIVE=1 pytest tests/integration_tests).
    if not (os.environ.get("TYPESEARCH_LIVE") == "1" and "integration_tests" in request.node.nodeid):
