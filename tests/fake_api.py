...
class FakeApi:
    ...
    def _handle(self, h: BaseHTTPRequestHandler) -> None:
        ...
        if route == "POST /v1/search":
            errors = sorted(_VALIDATORS["SearchRequest"].iter_errors(body), key=lambda e: list(e.absolute_path))
            if errors:
            ...
            return self._send(
                h, 200, search_response(body.get("max_results", 10), mode=body.get("mode", "normal"), queries=[body["query"]])
            )
